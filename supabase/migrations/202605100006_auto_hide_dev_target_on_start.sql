...
declare
  ...
  dev_hidden_count integer := 0;
  not_ready_names text[];
  player_count integer;
  remaining_hiders integer := 0;
...
begin
  ...
  get diagnostics dev_hidden_count = row_count;

  if dev_hidden_count > 0 then
    select count(*)
    ...
    if remaining_hiders = 0 then
      perform public.pe_release_seeker_internal(target_room_id, current_session_id, actor.id);
    end if;
  end if;

  return jsonb_build_object(
    'started', true,
    'gameSessionId', current_session_id,
    'devTargetAutoHidden', dev_hidden_count > 0
  );
end;
$$;

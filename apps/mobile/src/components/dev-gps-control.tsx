...
import { Platform, Pressable, type GestureResponderEvent, Text, View } from 'react-native';
...
export function DevGpsControl({ defaultDistance = 0, label }: { defaultDistance?: number; label: string }) {
  ...
  const updateFromPress = (event: GestureResponderEvent) => {
    const nextPercent = Math.min(1, Math.max(0, event.nativeEvent.locationX / railWidth));
    setDistance(Math.round(nextPercent * maxDistanceMeters));
    setActive(true);
  };
  ...
  return (
    <View
      ...
      <View style={{ alignItems: 'center', flexDirection: 'row', justifyContent: 'space-between', gap: 8 }}>
        <Text selectable style={{ color: colors.ink, flex: 1, fontSize: 12, fontWeight: '900' }}>
          DEV GPS: {label}
        </Text>
        <Pressable
          ...
          <Text selectable style={{ color: active ? colors.surface : colors.ink, fontSize: 11, fontWeight: '900' }}>
      ...
      <Pressable
        onLayout={(event) => setRailWidth(Math.max(1, event.nativeEvent.layout.width))}
      ...
      <View style={{ flexDirection: 'row', gap: 8 }}>
        {[0, 4, 8, 15, 35].map((preset) => (
          <Pressable
            ...
            <Text selectable style={{ color: distance === preset ? colors.surface : colors.ink, fontSize: 11, fontWeight: '900', textAlign: 'center' }}>
              {preset}m
            </Text>
          </Pressable>
        ))}
      ...
      <Text selectable style={{ color: colors.muted, fontSize: 11, fontWeight: '800', textAlign: 'center' }}>
        Distancia sintetica: {distance}m. Use 0m no escondido e 4m no procurador para capturar.
      </Text>
    </View>
  );
}
